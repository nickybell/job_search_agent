...
- [ ] **Publish (optional).** The repo is intended as a public portfolio piece
  ...
  (not ***REMOVED***). `base_resume.docx` stays gitignored.
...
- [x] **Application Tracker** (Step 5) — resolved: write-only Google Sheet `Job Search Application Tracker` (ID `<SPREADSHEET_ID>`), appended via the local `gws` CLI (personal `nicky.bell@gmail.com` OAuth, GCP project `***REMOVED***`) when the Step 4 packet is generated. 7 columns (Company/Title/URL/Date Posted verbatim; Date Added agent-set; Date Applied/Status user-filled with a validated, color-coded Status dropdown). See "Application Tracker (Google Sheets)" in `prd.md`.
