...
uv run jsa refetch --dry-run                              # report ATS drift on stored postings
uv run jsa refetch --id 24                                # re-read one posting and reconcile it
...
- **Four supported ATS = an inclusion criterion.** Greenhouse, Lever, Ashby, Rippling. Each has a distinct detail-record shape (see the per-platform fetchers in `ats/fetch.py`, verified live — e.g. Rippling's description is a `{role, company}` HTML dict, Lever falls back US→EU host, Ashby has no per-id GET so the org board is scanned). A URL that resolves to none of them is out of scope *for the search*. Any other platform (Workday, custom sites) has no supported index check — do not add per-platform verification for it.
- **Capture and liveness are separate concerns — don't re-conflate them.** The four-ATS rule earns its keep in the search prompt, where it is the liveness proxy; `ats/fetch.py` is pure capture and its failure never excludes a row. So the restriction was never load-bearing for capture. On the **manual path only**, capture falls back to schema.org JSON-LD (`ats/jsonld.py`) for unsupported platforms: order is supported fetcher → JSON-LD → `NULL`, never the reverse (Greenhouse and Lever publish no JSON-LD, so the platform fetcher is their only source). The **search path deliberately does not use the fallback** — a searched URL off the four platforms is a prompt violation with unverified liveness, and enriching it would disguise that. Never treat a successful JSON-LD capture as evidence a posting is live; a pulled req still renders it.
...
- **The tracker append uses `insertDataOption = OVERWRITE`, never `INSERT_ROWS`.** Not a style choice. `INSERT_ROWS` inserts a row that lands *outside* the Status column's data-validation and conditional-formatting ranges (so the appended row loses its dropdown) and shifts those ranges down by one each time, dragging them permanently off the data; the new row also inherits the header's bold grey. Measured on the live sheet after two real appends, corrected in `prd.md`, and pinned by a test asserting the exact `--params`. Don't "restore" `INSERT_ROWS`.
- **Postings mutate under a stable URL, so capture-once needs a reconciliation path.** `refetch.py` re-reads the ATS record and re-applies the insert's rule (ATS title wins, `title_slug` re-derived). Two invariants: a failed fetch leaves the row *completely* untouched (never trade a good capture for a blip — for a pulled posting the stored JD is the only surviving record), and tracked rows are excluded by default, since the write-only Sheet cannot be corrected from this side.
...
- `cli.py` → `refetch.run_refetch` reconciles stored rows against their ATS record: `db.rows_for_refetch` → `resolve_ats_url` → `fetch_detail` → `db.update_jd_capture` (the same updater the pipeline uses, so the title/slug rule stays in one place).
