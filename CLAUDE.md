...
- **The manual-add path reuses Step 2, it does not fork it.** `manual.py` runs the same canonicalize → `insert_posting` → `resolve_ats_url` → `fetch_detail` sequence, so a hand-added row is indistinguishable downstream (`search_agent = 'manual'` is the only marker). Two intentional departures, both load-bearing: it writes **no `search_findings` row** (that table is A/B search telemetry; a supplied posting would inflate an agent's coverage), and **an unsupported ATS is not a rejection** (the four-ATS rule is a liveness proxy for postings an agent found on its own — the user has already vouched for this one, so it inserts with a `NULL jd_markdown`).
