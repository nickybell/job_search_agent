...
| `title_slug` | Derived at insert time: a filesystem-safe rendering of the verbatim `title` (path separators and other reserved characters stripped/replaced, length-bounded). `title` itself stays verbatim; `title_slug` is what the Step 4 directory/filename builder consumes. |
...
gws sheets spreadsheets values append \
  --params '{"spreadsheetId":"<SPREADSHEET_ID>","range":"Applications!A:G","valueInputOption":"USER_ENTERED","insertDataOption":"INSERT_ROWS"}' \
