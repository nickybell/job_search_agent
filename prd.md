...
**Perplexity Agent API deep research (the recurring Mon/Wed/Fri search), the `xhigh` preset.** This requires an SDK-to-SDK connection with the API key supplied as a Fly-managed secret (`fly secrets set PERPLEXITY_API_KEY=…`), injected into the Machine as an environment variable at runtime.
...
- **Supported-ATS membership gates neither the insert nor the capture.** The four-ATS rule is an inclusion criterion for what the *search* may return — there it doubles as the liveness proxy for postings an agent found on its own and cannot otherwise vouch for. Here Nicky has already vouched, so liveness is settled and the only remaining question is whether the description can be retrieved. It usually can: Google requires schema.org `JobPosting` JSON-LD for a posting to surface in Google Jobs, so it is widely embedded server-side even on platforms whose human-facing pages are JavaScript shells (verified 2026-08-11 — a Workday detail page and an Ashby page both served a full `JobPosting` block to a plain GET with no JS). Capture therefore runs **supported ATS fetcher → JSON-LD → `NULL`**, and an unsupported URL only lands a `NULL jd_markdown` when both fail.
  ...
  The ordering is deliberate in both directions. The platform's own record wins where it exists: it carries a structured `location` and the ATS-canonical `title`, and it is the fuller text (4,471 chars via JSON-LD against 5,358 from the platform record, on one Workday req). And the fallback cannot substitute for it: Greenhouse and Lever publish no JSON-LD at all, so for those the platform fetcher is the only source there is.
...
For a *controlled* comparison both agents must run over the **same `{{SEARCH_WINDOW}}`**, which the bounded A/B trial arranged with a since-removed `jsa search --both` flag (Claude then Perplexity over one shared window and `now`, so `run_date` matched for both); `jsa ab-report` joins `search_findings` → `postings` to report per-agent coverage, overlap (both / claude-only / perplexity-only), and Apply precision.

**Trial outcome (2026-08-17).** Perplexity surfaced materially more qualifying opportunities than Claude — enough that its higher unique yield outweighs its lower Apply precision — while Claude still surfaced some roles Perplexity missed. So steady state weights Perplexity as the recurring Mon/Wed/Fri search and keeps Claude as a complementary weekly 168h sweep on Fridays (see Daily Search), rather than the earlier day-of-year alternation. `search_findings` and `jsa ab-report` remain: Friday still runs both agents (over different windows), so the attribution telemetry keeps accruing.
...
Step 5 appends one row to a Google Sheet for each job that reaches the application stage. It is the user's manual workspace for tracking outcomes.

**The relationship, reframed (2026-08-16).** Earlier revisions called the Sheet "write-only" and the write "append-only." Both labels conflated two ideas that needed separating: *where authority lives* and *which operations are permitted*. The intended principle was always the first one — **the database is the source of truth for posting data, and the Sheet is its human-readable projection plus the user's workspace** — and the operating rules follow from it:
...
*This supersedes the absolute "never reads back / only ever appends" rule, and the narrow read-only exception briefly recorded earlier the same day.* Everything those rules were protecting — no duplicated application state, no read-modify-write bookkeeping for the append, no Sheet-as-authority — still holds under the reframing.
