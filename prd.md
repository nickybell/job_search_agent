...
| `decided_at` | Timestamp the current `decision` was written — set on every decision write: the review loop, an in-session amendment, a manual add's INSERT, and a re-add promotion. This is what lets the prompt-refinement loop scope each run to ground truth that is *new since the last run* instead of re-reviewing the whole corpus. Nullable; a `NULL` on a decided row marks a decision already incorporated into the search prompt outside the loop's bookkeeping, and re-deciding the row refreshes the timestamp and returns it to scope. |
