...
**Why Perplexity anchors.** Perplexity surfaces materially more qualifying opportunities than Claude — its higher unique yield outweighs its lower Apply precision — while Claude still surfaces some roles Perplexity misses. So Perplexity carries the recurring Mon/Wed/Fri load, with Claude as the complementary weekly 168h sweep.
...
- **Model and effort:** `claude-opus-4-8` with `output_config: {effort: "xhigh"}` and adaptive thinking (`thinking: {type: "adaptive"}`), generous `max_tokens` headroom (≥ 64K). `xhigh` is the documented sweet spot for agentic search (lower effort consolidates/skips tool calls — wrong for a task whose value is exhaustive source-checking). Since Steps 1–2 run on the Claude Agent SDK (the Claude Code harness), `xhigh` is already its default effort on Opus — implementation amounts to pinning the model string.
...
- **No `search_findings` row.** That table is *search* telemetry. A posting the user supplied is not a search result, and crediting it to an agent would inflate that agent's coverage and distort Apply precision.
- **Supported-ATS membership gates neither the insert nor the capture.** The four-ATS rule is an inclusion criterion for what the *search* may return — there it doubles as the liveness proxy for postings an agent found on its own and cannot otherwise vouch for. Here the user has already vouched, so liveness is settled and the only remaining question is whether the description can be retrieved. It usually can: Google requires schema.org `JobPosting` JSON-LD for a posting to surface in Google Jobs, so it is widely embedded server-side even on platforms whose human-facing pages are JavaScript shells (Workday and Ashby detail pages both serve a full `JobPosting` block to a plain GET with no JS). Capture therefore runs **supported ATS fetcher → JSON-LD → `NULL`**, and an unsupported URL only lands a `NULL jd_markdown` when both fail.

  **A JSON-LD capture is evidence of content, never of liveness.** A pulled or unlisted req can still render perfectly good JSON-LD, so nothing may read a successful extraction as proof a posting is open — precisely the failure mode described under Daily Search. This is why the *search* path deliberately does **not** use the fallback: a searched URL outside the four platforms is a prompt violation whose liveness is unverified, and enriching it with a description would disguise that. The fallback exists only where a human has already vouched for the URL.

`company` and `title` are `NOT NULL`, so they are resolved in precedence order: an explicit `--company` / `--title` flag, then the ATS detail record's canonical title and the board slug (the board slug *is* the company, just lowercased and hyphenated), then — in an interactive session — whatever the user edits into a pre-filled prompt. An explicit flag is never overwritten by the ATS transcription, which is the one place this path departs from Step 2's "the ATS title wins" rule: a deliberate override outranks a transcription. A non-interactive add (`--no-input`) that can derive neither field fails loudly rather than inventing a placeholder.
...
| `search_agent` | Which agent *first* inserted the posting (`claude` / `perplexity`), or `manual` for a posting the user added directly (see Direct Job Add). Because both agents converge on the same `canonical_url`, this column alone can't show overlap — full per-agent attribution lives in `search_findings` (below). |
...
| `decided_at` | Timestamp the current `decision` was written — set on every decision write: the review loop, an in-session amendment, a manual add's INSERT, and a re-add promotion. This is what lets the prompt-refinement loop scope each run to ground truth that is *new since the last run* instead of re-reviewing the whole corpus. Nullable; a `NULL` on a decided row simply means it predates this column. On the loop's first-ever run (no prior run recorded) those NULL rows are genuinely new ground truth and are included; once a run has been recorded, `decided_at > cutoff` naturally excludes NULL rows going forward, and re-deciding the row refreshes the timestamp and returns it to scope. |
| `added_to_tracker` | Boolean. `1` (True) indicates that the row has been added to the application tracker in Google Sheets.

**`search_findings` (search telemetry).** A second, append-only table records one row per `(run_date, agent, canonical_url)` for *every* posting an agent returns — including postings whose `postings` insert no-ops — because `search_agent` on the posting row records only the *first* inserter and therefore cannot show overlap. It keeps accruing under the weekly cadence, since Friday runs both agents. It is search telemetry, not application state — which is why the direct job-add path deliberately writes no row here (see Direct Job Add).

Each row also carries a `decision` column, denormalized from the matching `postings.canonical_url` rather than joined at evaluation time: per-agent coverage, overlap, and Apply precision can be computed straight off `search_findings` for external search-quality evaluation. The pipeline deliberately ships no built-in report over this data — the table is retained as raw evaluation telemetry only. Denormalizing the decision is what lets `postings` rows be deleted or reset without corrupting search-quality evaluation — a req a `postings` row no longer exists for (pruned as out of scope) or has been reset to undecided (sent back for re-review) still counts correctly toward the agent that found it. `db.sync_finding_decision` keeps it current, called from `record_decision` and `set_decision` whenever a decision is actually made; `clear_decision` (resetting a posting to undecided) deliberately does not call it, so the last real decision stays as the historical record until the posting is re-decided.

**`prompt_refinement_runs` (the refinement loop's cursor).** One row per prompt-refinement run that considered ground truth: an `id` primary key (autoincrement), `run_at`, the number of rows considered, and whether a change was proposed. The `id` exists so a run row is individually deletable — libSQL over the Hrana/HTTP transport refuses a DELETE against a table with no stable row identity, and rolling a run back is how the ground-truth radius is widened before the next pass. The next run's scope is rows with `decided_at > MAX(run_at)`. A run is recorded whether or not its PR is merged — a rejected translation is still *considered* ground truth, and the way to resurface a posting is to re-decide it. This table, not the prompt, is where incrementality lives: the search prompt itself stays standalone (see Search Prompt Updates from "Ground Truth").

**`bullet_sync_runs` (the bullet-library sync's cursor).** One row per `jsa bullets` run, with the same shape and rationale as `prompt_refinement_runs`: an autoincrement `id` (rows stay individually deletable over Hrana/HTTP), `run_at`, the number of resumes considered, and whether the CSV changed. The next run's scope is resume files with an mtime newer than `MAX(run_at)`; a run is recorded only on success (an errored run's resumes are reconsidered), and `jsa bullets --baseline` records a row without a model call — the seed step after the library is built or hand-curated (see Resume Revisions).

**Connection contract.** The client is `turso_serverless` (DB-API 2.0 over Turso's HTTP/Hrana transport — the managed platform does not serve WebSockets, so the native and WebSocket clients do not fit); a `file:` URL opens a local SQLite file through the standard library for throwaway development, and the rest of the layer treats both identically. `db.connect` enables autocommit with `isolation_level = None`. The client also exposes a DB-API `autocommit` attribute that is stored but never consulted, so without the isolation setting every write opens an implicit transaction that `close()` rolls back — a silent-data-loss trap that once ate a whole run. Both the pipeline inserts and the review loop depend on this.

**A schema change is a migration, not an edit to the CREATE statement.** `CREATE TABLE IF NOT EXISTS` no-ops against the live database, and SQLite cannot ALTER a CHECK constraint, so `db.migrate_postings_schema` rebuilds the table (create → copy → rename → rename → drop, ordered so the data is never absent from a table named `postings`, with an interrupted swap recovered on the next open) and `init_db` calls it *before* its CREATE statements, so every command applies it. Additive columns (like `decided_at`) go through the same function as a plain `ALTER TABLE ADD COLUMN`, ordered before the rebuild so the rebuild's named copy list finds them. The column list lives in one place (`_POSTINGS_COLUMNS`) so the live schema and the migration target cannot drift.
...
Step 4 produces a per-job resume tailored from a **curated template library**, alongside a changelog recording what the tailoring changed and why. It runs **asynchronously to Step 3**: deciding to `Apply` in the review loop does not trigger resume work inline — it only records the decision and lets the row accumulate in a queue that Step 4 drains on demand.

**The template library.** `resume_templates/` (gitignored — personal data; `JSA_RESUME_TEMPLATES_DIR` to override) holds one maintained `.docx` per role family — e.g. `customer-education.docx`, `ai-enablement.docx` — with the filename slug as the template's id. One template per role family because the right starting point for tailoring is the version already aimed at the job's family: per-job deltas stay small, and the patch never has to restructure across families. A small *curated* library — rather than patching from whichever previously tailored resume is nearest — is what prevents copies-of-copies drift, per-company phrasing leaking between applications, facts needing fixes in N leaves, and the loss of refetch's regenerate-by-construction guarantee. Two maintenance rules:
...
2. **Selects the base template and revises it for this posting in one pass**, using the JD as context and the tailoring instructions (below), and emits the tailored resume in both formats into the directory. The mechanism is a **structured patch driven through a render loop**: the model call — the Claude Agent SDK, headless, pinned to **`claude-fable-5` at `medium` effort** (revision quality proven out in practice justifies the top tier for this quality-critical, low-volume output; the bounded render loop needs no more than medium effort) — never edits the `.docx` itself. It is given *every* template's numbered paragraph text plus the JD, and a single in-process tool, `render_resume`, that takes its JSON patch, applies it to a fresh copy of the chosen template with `python-docx`, renders the PDF, and reports the page count and each page's text back. The patch names the chosen base (`base`: a template slug — **the model picks the template**, and the pick plus its rationale land in the changelog where a wrong pick is visible and correctable on a re-run), optionally declares a new role family (`new_family`: triggers the library-expansion rule above), and lists per-paragraph operations — `replace` a paragraph's text, `insert_after` a new paragraph (which inherits the anchor's formatting, so an inserted bullet is a bullet), or `delete` one, with bold carried inline as `**…**` — each anchored on an id from the chosen template's original numbering. The model iterates the patch against the tool until the resume satisfies the **hard two-page budget** with no orphaned role headers; the last render is the packet's resume. The apply step is deterministic (formatting is inherited from the template's own runs, never synthesized) and the changelog falls out of the applied patch for free, but the loop itself is agentic — reruns are not bit-identical, an accepted trade for letting the model verify the page budget against a real render rather than guessing at it from the numbered text. The PDF the tool renders on each pass uses LibreOffice headless (`soffice --headless --convert-to pdf`) — fully scriptable and detachable, no UI automation, fonts resolved from the local machine. Output files:
   - `{JSA_CANDIDATE_NAME}_Resume_{title_slug}_{normalized_company}.docx`
   - `{JSA_CANDIDATE_NAME}_Resume_{title_slug}_{normalized_company}.pdf`
   - **Space characters are removed from the resume `.docx`/`.pdf` file names, but not from the containing directory name.** The candidate prefix comes from the `JSA_CANDIDATE_NAME` environment variable and is omitted when it is unset.
...
The resulting directory is the self-contained application packet for one job: the source posting, the tailored `.docx`/`.pdf` pair, and the changelog. Each eligible row is a fully independent unit of work — its own directory, its own SDK agent loop, no shared state — so `jsa generate` **processes the queue in parallel** (a bounded worker pool, not one row at a time). The pool is capped (default 3 workers, `JSA_GENERATE_WORKERS` to override) so a large backlog cannot fan out unbounded model calls; `--id` collapses to a single row. The closing tracker appends are serialized across workers, preserving Step 5's one-row-at-a-time append semantics.
...
**Tailoring instructions.** These live in a versioned template file paralleling `deep_research_prompt.md` (the search prompt) — `tailoring_prompt.md` — so they are diffable, reviewable, and eventually reachable by the ground-truth loop. The file is a template, interpolated per job with the posting's title, company, and JD (plus every template's numbered text and the bullet ground-truth library below), and it carries inline both the op-based patch contract (`replace`/`insert_after`/`delete`/`move`, with `**bold**` spans) and the render-loop workflow: submit a patch to `render_resume`, read the returned page count and per-page text, and iterate until the resume fits two pages with no orphaned role headers — the same pattern as the search prompt carrying its own output contract.

**The summary is written in the candidate's register, enforced structurally.** Comparing the generated summaries against the versions actually submitted across ten applications showed the same edits every time: the summary got shorter (roughly 90 words to 60), the sentence that restated the posting's responsibilities in the first person was deleted, every contrast construction ("rather than X", "unlike most CSMs") was removed, tool and metric names were stripped, and the identity credential was named in a few words ("brought an FDA-cleared AI medical device to market") rather than narrated as a bullet. The prompt already carried soft guidance against most of these and the model ignored it, so `tailoring_prompt.md` states them as hard constraints the model checks against the rendered text: at most three sentences and about 60 words (75 ceiling), no responsibilities sentence, no contrast constructions, no product/vendor/metric names, identity-not-proof. The register itself is transmitted by example: `resume_voice.md` — in the packets directory beside the bullet library (`JSA_VOICE_DOC` to override) — holds summaries the candidate wrote by hand for earlier applications, interpolated as `{{VOICE_SAMPLES}}`. It is hand-curated only (no sync command; the user seeds and prunes it), and a missing file degrades to a placeholder note. Candidate-specific constraints — a protected entry that must never be tailored, a title-wording rule for one role — live the same way in `resume_tailoring_rules.md` beside it (`JSA_TAILORING_RULES_DOC` to override), interpolated as `{{TAILORING_RULES}}` and overriding the general guidance where the two conflict; the general instructions in `tailoring_prompt.md` never name an employer, so the prompt is candidate-agnostic and a fork changes only the vault files.

**The bullet ground-truth library.** `resume_bullets.csv` — living in the packets directory beside the resumes it indexes (`JSA_BULLETS_DOC` to override) — is the reconciled universe of claims across every resume actually sent: one row per bullet, organized by employer role in resume order, with a `claim` label grouping wordings of the same achievement, one canonical `best` wording per claim, substantively different framings kept in full as `variant` rows tagged by the role categories they serve, and near-duplicate wordings collapsed rather than accumulated. The tailoring prompt interpolates the whole CSV as `{{BULLET_LIBRARY}}`: the model reuses the library's vetted wordings (the `best` version by default, the category-matched variant when it fits the posting) rather than re-paraphrasing claims per run — the drift the curated template library prevents *across role families*, the bullet library prevents *across applications*. A missing library degrades to a logged warning, never a failed run.

Keeping it current is `jsa bullets`, on the refinement loop's pattern — **incrementality lives in the database**: each run scopes to resume `.docx` files modified since the last row in `bullet_sync_runs` (first run: every resume; a hand-edited resume re-enters by mtime), pre-renders their text (bold preserved as `**…**`), and drives a headless vault-editing agent — pinned **`claude-sonnet-5` at `medium` effort**, Read/Edit tools over the vault only: the task is classification and transcription against explicit reconciliation rules, not synthesis, so it needs no capability above Sonnet — that folds missing bullets into the CSV under the rules in `bullet_sync_prompt.md` (exact match → annotate sources and categories; substantive variant → a new `variant` row; new claim → a new `best` row; never delete, never invent, never merge distinct claims). A run records itself only on success, so an errored run's resumes are reconsidered next time; `--baseline` records the current state as synced without a model call — the seed step after the library is first built or hand-curated. The CSV lives outside the repo (personal data, the same stance as `resume_templates/`).
...
| Column | Source |
| --- | --- |
| `ID` | `id` — the database primary key, written at append time. The join key that lets `jsa refetch`'s scope lookup match a Sheet row back to its `postings` row without comparing URLs. |
...
`OVERWRITE` writes into the sheet's already-existing blank rows rather than inserting. Sheets still locates the table server-side and writes after its last row — no row-index bookkeeping, no read-modify-write — while the written row keeps the validation, conditional formatting and default styling those rows already carry.

### Search Prompt Updates from "Ground Truth"

The search prompt is refined from Step 3's accumulated ground truth by an **automated weekly cron whose output is a pull request, never a direct commit**. The instructions live versioned in the repo as `refine_search_prompt.md`, so the same pass is runnable by hand (`jsa refine`) as the escape hatch. The refiner is pinned to **`claude-opus-4-8` at `high` reasoning effort** — the task is bounded synthesis over a small corpus delta, not the exhaustive source-checking that justifies `xhigh` on the search side.

**Scope: only what is new.** Each run considers the decided rows with `decided_at` later than the last recorded run (`prompt_refinement_runs`) — on the very first run, with no prior run to compare against, that means every decided row, including ones with a NULL `decided_at` from before the column existed — and exits quietly when there are none. It does not re-review the whole ground-truth corpus; aggregate SQL over history (counts, co-occurrence checks to confirm a pattern is really recurring) is fine, wholesale re-reading is not.
...
2. **Manual adds as the recall set.** Every `jsa add` (`search_agent = 'manual'`, decided Apply, usually no feedback text) is a role the user found that the search did not. For each one that carries a captured JD, the refiner judges whether the prompt as written would have surfaced it and classifies the miss: a title-vocabulary gap, a source gap, or an over-tight negative signal, each fixable with a prompt edit. A manual add with **no captured JD** — almost always a posting on an ATS outside the supported four, which the search cannot emit by construction — is simply **ignored**: hand-adding an off-platform role is an ordinary part of the workflow, not a search defect, and with no JD there is nothing to mine or encode, so the refiner records nothing for it.
...
- **A negative signal that never yields an Apply earns promotion.** When a negative-signal category recurs across rounds with zero Applies, the refiner proposes promoting it to a hard exclusion — or, where posting-verifiable, a filter — flagged prominently in the PR body. This is the one sanctioned trade of recall for precision, and the PR review is its guard: a wrong promotion costs one rejected PR, not silent lost recall.
...
**Coverage is not success — every round audits adherence.** The measure of a refinement round is the precision of the surfaced set, not whether every skip reason has a rule; a skip reason already being encoded is the starting point for asking why the posting was surfaced anyway. The refiner classifies every Skip in scope: covered by no rule (a coverage gap, fixable under the taxonomy), covered by a negative signal (expected under recall-first, tallied as the promotion rule's evidence), or in violation of a hard filter or hard exclusion — an adherence failure by the search agent, never "working as designed." Adherence failures are fixed with salience edits — repositioning, restating, or consolidating the ignored rule, or adding a pre-emit self-check instruction to the search prompt — and where no wording change can plausibly help, the refiner proposes runner-side enforcement in the PR body's open questions instead. A row decided under an older prompt version that predated the rule it seems to violate is weighed before being called an adherence failure.

**The prompt stays standalone.** `deep_research_prompt.md` is downstream of the ground truth but never refers to it: no watermark, no incorporated-through marker, no provenance annotations in the prompt text. Consequence, accepted: without in-prompt memory of which feedback produced which line, successive runs can oscillate wording. Provenance lives where it belongs — each run is one PR with its rationale, so git history traces every rule to the round that introduced it and a rule that hurt yield is revertible.

**The guardrail is the PR — deliberately no sentinels, no pinning test.** The refiner's edit lands as a pull request carrying the rationale and diff, and the user merges; that human review *is* the protection for the load-bearing machinery. The refiner's instructions (`refine_search_prompt.md`) carry the do-not-touch list — the JSON output contract, the `{{SEARCH_WINDOW}}` placeholder, the liveness/verifiability section and its ATS index-check table — and the diff is small enough that a violation is obvious at review. Two further instructional guardrails: **recall-first survives every round** (edits reduce wasted review time and never create false negatives on roles the user would want — the one sanctioned trade is an evidence-backed promotion, prominently flagged in the PR for veto), and genuine contradictions or ambiguities — which an interactive session would resolve with a clarifying question — go into the PR body as open questions, since the cron has no one to ask — including anything too ambiguous to encode, with the reasoning.
...
- *Cloud (Fly.io):* Steps 1–2; writes to Turso.
- *Cloud (GitHub Actions):* the weekly prompt-refinement loop; reads Turso, edits a repo checkout, opens the PR.
